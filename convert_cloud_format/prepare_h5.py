...
def room_to_blocks(cloud,  size, stride, threshold, num_points):
    # cloud = np.load(fname)
    # num_points = int((cloud.shape[0])/3)
    cloud[:, 3:6] /= 255.0
    ...
    for (x, y) in cells:
        ...
        if np.sum(cond) < threshold:
            continue
        block = cloud[cond, :]
        if max(block[:, 0]) - min(block[:, 0]) < 0.5 * size or max(block[:, 1]) - min(block[:, 1]) < 0.5 * size:
            continue
    ...
    batch = np.zeros((num_blocks, num_points, 13))
...
#         continue
#
#     batch = room_to_blocks(fname, size=1.0, stride=0.5)
